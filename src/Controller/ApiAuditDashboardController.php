...
{
    private const REQUEST_LOOKUP_PAGE_CONFIG = [
        'page_url' => '/index.php/consulta-requisicoes',
        'page_title' => 'Consulta de Requisições',
        'page_subtitle' => 'Pesquisa completa das chamadas gravadas, com foco em request, response e rastreabilidade por request_id.',
        'page_heading' => 'Consulta de Requisições',
        'heading_text' => 'Página dedicada para localizar requisições e inspecionar request, response, erros, tentativas e eventos operacionais completos.',
        'breadcrumb_label' => 'Consulta de requisições',
        'storage_prefix' => 'consulta_requisicoes',
        'export_filename_prefix' => 'consulta_requisicoes',
    ];
    ...
    #[Route('/consulta-requisicoes', name: 'app_api_request_lookup', methods: ['GET'])]
    public function requestLookup(Request $request): Response
    {
        return $this->renderAuditDetailPage($request, self::REQUEST_LOOKUP_PAGE_CONFIG);
    }
    ...
    /** @param array<string,string> $pageConfig */
    private function renderAuditDetailPage(Request $request, array $pageConfig): Response
    {
        [$filters, $limit, $sort, $direction, $page, $summary, $advancedMetrics, $comparison, $alerts] = $this->buildAuditViewState($request);
        ...
        if ((string) $request->query->get('csv', '0') === '1') {
            return $this->buildCsvResponse($pageConfig['export_filename_prefix'] . '.csv', $this->repository->findRequests($filters, 10000, 0, $sort, $direction));
        }
        if ((string) $request->query->get('xlsx', '0') === '1') {
            return $this->buildXlsxResponse($pageConfig['export_filename_prefix'] . '.xlsx', $this->repository->findRequests($filters, 10000, 0, $sort, $direction));
        }
        ...
        [$previousRequestId, $nextRequestId] = $this->resolveNeighborIds($requests, $selectedRequest !== null ? (string) $selectedRequest['u_c_request_id'] : null);
        ...
        return $this->render('catalog/api_audit_dashboard.html.twig', [
            ...
            'auditPageUrl' => $pageConfig['page_url'],
            'auditPageTitle' => $pageConfig['page_title'],
            'auditPageSubtitle' => $pageConfig['page_subtitle'],
            'auditPageHeading' => $pageConfig['page_heading'],
            'auditPageHeadingText' => $pageConfig['heading_text'],
    ...
    /** @param array<string,int> $summary @param array<string,mixed> $advancedMetrics @param array<string,mixed> $comparison @return list<array<string,string>> */
    private function buildAlerts(array $summary, array $advancedMetrics, array $comparison): array
    {
        ...
        if ($currentErrorRate >= 20 || $errorRateDelta >= 10) {
            $alerts[] = ['nivel' => 'danger', 'titulo' => 'Taxa de erro acima do normal', 'mensagem' => sprintf('A taxa de erro atual está em %d%% e variou %d pontos contra o período anterior.', $currentErrorRate, $errorRateDelta)];
        }
        if ($currentLatency >= 3000 || $latencyDeltaPerc >= 35) {
            $alerts[] = ['nivel' => 'warning', 'titulo' => 'Latência média em alta', 'mensagem' => sprintf('O tempo médio atual está em %d ms e variou %d%% contra o período anterior.', $currentLatency, $latencyDeltaPerc)];
        }
        if ($queued > 0 || $processing > 0) {
            $alerts[] = ['nivel' => 'warning', 'titulo' => 'Fila ativa', 'mensagem' => sprintf('Existem %d requisições enfileiradas e %d em processamento nos filtros atuais.', $queued, $processing)];
        }
        if ($alerts === []) {
            $alerts[] = ['nivel' => 'success', 'titulo' => 'Sem alertas críticos', 'mensagem' => 'Não houve pico relevante de erro, latência ou fila no recorte atual.'];
        }

        return $alerts;
    }

    /** @param array<string,mixed> $filters @return array{0:array<string,mixed>,1:array<string,mixed>,2:string,3:string} */
    private function deriveComparisonFilters(array $filters): array
    {
        ...
        return [$currentFilters, $previousFilters, sprintf('%s a %s', $start->format('d/m/Y'), $end->format('d/m/Y')), sprintf('%s a %s', $previousStart->format('d/m/Y'), $previousEnd->format('d/m/Y'))];
    }
    ...
    private function buildDelta(int $current, int $previous, bool $points): array
    {
        ...
        return ['diff' => $diff, 'percent' => $percent, 'formatted' => sprintf('%s%d%s', $diff >= 0 ? '+' : '', $diff, $points ? ' pts' : ''), 'trend' => $diff > 0 ? 'up' : ($diff < 0 ? 'down' : 'flat')];
    }

    /** @param list<array<string,mixed>> $rows @return array<string,mixed> */
    private function buildVerticalSvgChart(array $rows, string $labelField, string $valueField): array
    {
        $chartWidth = 320.0; $chartHeight = 180.0; $leftPadding = 18.0; $rightPadding = 18.0; $topPadding = 26.0; $bottomPadding = 34.0; $plotHeight = $chartHeight - $topPadding - $bottomPadding;
        $max = 0;
        foreach ($rows as $row) { $max = max($max, (int) ($row[$valueField] ?? 0)); }
        $bars = []; $count = max(1, count($rows)); $slot = ($chartWidth - $leftPadding - $rightPadding) / $count; $barWidth = max(12.0, $slot - 8.0);
        foreach ($rows as $index => $row) {
            $value = (int) ($row[$valueField] ?? 0); $height = $max > 0 ? ($value / $max) * $plotHeight : 0.0; $x = $leftPadding + ($index * $slot) + (($slot - $barWidth) / 2); $y = $chartHeight - $bottomPadding - $height;
            $bars[] = ['x' => round($x, 2), 'y' => round($y, 2), 'width' => round($barWidth, 2), 'height' => round($height, 2), 'label' => (string) ($row[$labelField] ?? ''), 'value' => $value, 'label_x' => round($x + ($barWidth / 2), 2), 'label_y' => round($chartHeight - 12.0, 2), 'value_y' => round(max(14.0, $y - 6.0), 2)];
        }
        return ['bars' => $bars, 'max' => $max, 'width' => (int) $chartWidth, 'height' => (int) $chartHeight, 'axis_y' => (int) ($chartHeight - $bottomPadding)];
    }

    /** @param list<array<string,mixed>> $rows @return array<string,mixed> */
    private function buildHorizontalSvgChart(array $rows, string $labelField, string $valueField, string $suffix = ''): array
    {
        $chartWidth = 420.0; $labelWidth = 132.0; $valueWidth = 74.0; $leftPadding = 8.0; $rightPadding = 12.0; $topPadding = 10.0; $rowHeight = 28.0; $barHeight = 14.0; $plotWidth = $chartWidth - $labelWidth - $valueWidth - $leftPadding - $rightPadding;
        $max = 0; foreach ($rows as $row) { $max = max($max, (int) ($row[$valueField] ?? 0)); }
        $bars = [];
        foreach ($rows as $index => $row) {
            $value = (int) ($row[$valueField] ?? 0); $y = $topPadding + ($index * $rowHeight); $barWidth = $max > 0 ? round(($value / $max) * $plotWidth, 2) : 0.0;
            $bars[] = ['x' => $labelWidth, 'y' => $y, 'width' => $barWidth, 'label' => $this->shortenLabel((string) ($row[$labelField] ?? ''), 28), 'full_label' => (string) ($row[$labelField] ?? ''), 'value' => $value . $suffix, 'label_x' => $leftPadding, 'label_y' => $y + 10.5, 'value_x' => $labelWidth + $barWidth + 8.0, 'value_y' => $y + 10.5, 'height' => $barHeight];
        }
        return ['bars' => $bars, 'max' => $max, 'width' => (int) $chartWidth, 'height' => (int) max(86.0, $topPadding + (count($rows) * $rowHeight))];
    }
    ...
    {
        $max = 0;
        foreach ($rows as $row) { $max = max($max, (int) ($row[$metricField] ?? 0)); }
        foreach ($rows as &$row) {
    ...
    {
        if ($selectedId === null || $selectedId === '') { return [null, null]; }
        ...
        if ($index === false) { return [null, null]; }
